...
drives Eleva's recurring workflows. It mirrors `infra/stripe/` —
vendor-scoped operator scripts that talk to one external system.

## Why a workspace and not just app scripts?

The schedules POST to `apps/api`, but the **provisioning tool** is
QStash. Keeping it under `infra/` (alongside Stripe and flags) makes
the "external system → operator scripts" map symmetric and avoids
tying our deploy automation to the layout of any single app.
...
| `pnpm qstash:setup:all`           | Run both setup commands in sequence               |

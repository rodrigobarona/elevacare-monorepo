...
export const consents = pgTable(
  ...
  (t) => ({
    ...
    subjectChk: check(
      "consents_subject",
      sql`(subject_kind = 'user' AND user_id IS NOT NULL) OR (subject_kind = 'guest' AND guest_email_hash IS NOT NULL) OR (booking_id IS NOT NULL AND user_id IS NULL AND guest_email_hash IS NULL AND subject_pseudonym IS NOT NULL)`
    ),

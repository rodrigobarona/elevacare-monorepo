...
import { Tooltip, TooltipTrigger } from "@eleva/ui/components/tooltip"
import { SidebarIcon } from "@eleva/icons"
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // eleva: never hijack Cmd/Ctrl+B while typing (inputs, rich-text editors use
      // it for bold). The registry version has no editable-target guard.
  ...
  if (isMobile) {
    return (
      <Sheet
        ...
        data-slot="sidebar"
        data-mobile="true"
        // eleva: the close button lives inside the inner Dialog, so `[&>button]:hidden`
        // on the panel never matched; use the Sheet API instead.
...
// eleva: keep list semantics (`ul`/`li`) so screen readers announce the navigation as a
// list with item counts and it matches `SidebarMenuSub`. The registry renders `div`s.

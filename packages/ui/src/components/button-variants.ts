...
// eleva: must stay in a file without `"use client"` so Server Components can
// call `buttonVariants()` on `<Link className={buttonVariants()}>`. Keep this
// split after `shadcn add --overwrite button`. `cursor-pointer` /
// `disabled:cursor-not-allowed` are a product decision (PR #17).

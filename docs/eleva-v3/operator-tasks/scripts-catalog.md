...
| `WORKOS_API_KEY`           | `workos:rbac:generate`, `stripe:backfill:org-customers`                   |
...
4. **Update `STRIPE_WEBHOOK_SECRET`** on Vercel `elevacare-api` Production with the new whsec, then redeploy so the route picks it up.
5. **Backfill orgs to live customers**:
   ```bash

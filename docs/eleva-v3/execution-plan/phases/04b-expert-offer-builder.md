...
- [x] Authenticated plural `/experts/{profile,schedule,event-types,integrations}`
...
6. packages/editor (new, ADR-023): pnpm exec shadcn add for the Plate editor-basic block and the
   ...
   packages/db/src/schema/main/shared.ts types + localizedRichTextSchema in
   @eleva/editor — Zod stays in packages/editor so platejs stays behind ADR-023):

...
- `transcriptReady` — Daily transcript webhook (`POST /webhooks/daily`) → store Eleva record (encrypted via `@eleva/encryption`, ADR-020) → enqueue draft
...
- Transcripts are Eleva-owned records encrypted via `@eleva/encryption` (ADR-020). WorkOS Vault (removed, see ADR-017).

...
| POST   | `/bookings/reserve`                                   | public + BotID + rate limit | Guest or session; consents required. Private-link bookings must send `linkToken`; the server validates hash, expiry, revocation, `max_uses`, and recipient, and returns the same `404` on any failure |
| POST   | `/payments/intent`                                    | public + BotID + rate limit | `{ reservationId, reservationToken }`. After reserve, `reservationToken` is the only client credential. A revoked stored `booking_link_id` returns `404`                                              |
| POST   | `/bookings/confirm`                                   | public + BotID + rate limit | `{ reservationId, reservationToken, paymentIntentId }`. `reservationToken` required (Zod `400`). Same stored-link revocation `404` as intent                                                          |

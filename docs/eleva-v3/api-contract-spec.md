...
| GET    | `/me/bookings/{id}/cancellation-quote` | session / bearer           | Refund the member would get if they cancelled now: `policy`, `refundPercent`, `refundCents`, `reason` (`grace_period` \| `policy_tier`), `fullRefundUntil`, `nextChangeAt`. **409** `POLICY_TOO_LATE` once the session started     |
| POST   | `/me/bookings/{id}/cancel`             | session / bearer           | Any time before `starts_at`. Refund from the booking's snapshotted cancellation policy; response includes `refund`. Status `cancelled`, slot released, `refund_due_cents` set; `refund_pending` only when the refund is above zero |
| POST   | `/me/bookings/{id}/reschedule`         | session / bearer           | Only while a cancel would still refund 100% and within the service's reschedule window. Body `{ startsAt, endsAt }`. Moves times, releases the old slot, ICS via existing helpers                                                  |

...
SSOT remains OpenAPI at `GET /openapi.json` and Zod in `@eleva/api-client`. Session cookie or Bearer. `RATE_LIMITS.authenticated` on `/me` and `/privacy`. Never weaken `RATE_LIMITS.public` (10/min). BotID is not used on these authenticated member routes.
...
| GET    | `/me/bookings`                      | session / bearer           | `range` upcoming or past, cursor. Reads via `withPlatformAdminContext` + `member_user_id`                                                                   |
...
| POST   | `/privacy/dsar`                     | session / bearer           | Creates `dsar_requests` (`pending`). Kickoff via `after()` + QStash (inline on localhost)                                                                   |

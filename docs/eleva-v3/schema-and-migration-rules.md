...
`email_suppressions` and `phone_verifications` are also not tenant-scoped: the
suppression list is a global mailbox namespace (citext unique, no org), and OTP
hashes are user-scoped secrets visible only to the notification worker.
...
| `notifications`                  | owner-user-visible SELECT/UPDATE(`read_at`) + staff/service INSERT/DELETE |
| `notification_deliveries`        | tenant-owned SELECT + service-only writes                                 |
| `email_suppressions`             | service-only                                                              |
| `phone_verifications`            | service-only (OTP `code_hash` is never owner-visible)                     |

...
| GDPR              | Art. 17 erasure               | Envelope crypto-shred (`shredOrgKeys` deletes DEK rows, ADR-020) removes readability of encrypted domain data. Audit retention is a documented exception under Art. 17(3)(b) (accountability / Art. 5(2) and Art. 30 records of processing, plus ERS-aligned 10-year clinical-record duty for health-adjacent operations) and Art. 17(3)(e) (establishment, exercise or defence of legal claims: disputes, chargebacks, regulator inquiries). Minimum fields = the allowlist (`audit_id` — mandatory, the drainer idempotency key — plus `entity`, `action`, `entityId`, `orgId`, `actorId`, correlation ids — no free-text email/name/PHI). The allowlist is the shipped audit **payload**; `audit_id` is also a first-class column. Period = 10 years from `created_at`. Any legacy payload field outside the allowlist is deterministically redacted to `REDACTED` before the drainer ships. DSAR export may include the redacted audit rows; Neon DBA `DELETE` on `audit_events` is itself audited and is not a self-service erasure path. |
...
| ------------------------------------------- | ------------------- | ----------------------------------- | --------------------------------------------------------------- |
| Better Auth (self-hosted)                   | EU (Neon)           | auth, orgs, sessions, API keys      | yes (identity; OAuth tokens encrypted by Better Auth)           |
...
| -------------------------------------- | -------------------------------- | ------------------------------------------------------------------- |
| Session notes                          | 10 years (ERS-aligned)           | soft-delete + 30-day scrubber + DEK shred on org deletion (ADR-020) |
...
| Uploaded documents                     | 10 years                         | soft-delete + DEK shred on org deletion (ADR-020)                   |
...
- Minimize access in both UI and backend service layers; enforce via Neon RLS + Better Auth capabilities (ADR-021) + app permission gates.

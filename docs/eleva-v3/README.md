...
> It is the sequencing SSOT: locked decisions (ADR-017..021, Better Auth is the target identity layer; the previous identity provider runtime is deleted in Phase 3 — removed, see ADR-017),
...
- **Better Auth** (self-hosted in `apps/api` at `/auth/*`) is the identity, session, organization and API-key layer (ADR-017).
...
- **Eleva-owned Google/Microsoft calendar OAuth** in `packages/calendar`; tokens from Better Auth `account` rows (ADR-017).
...
- [`operator-tasks/`](./operator-tasks/): one-shot cutover and provisioning checklists tied to specific releases ([Sprint 0 Vercel setup](./operator-tasks/s0-vercel-setup.md), [Stripe Phase 1+2 cutover](./operator-tasks/stripe-phase-1-2-cutover.md), [TOConline setup](./operator-tasks/toconline-setup.md), [Operator scripts catalog](./operator-tasks/scripts-catalog.md))

...
| `E2E_AUTH_CAPTURE`      | Redis `e2e:auth-url:*`        | Local/preview only. When `1`, Better Auth persists verify/reset/magic URLs in Upstash for Playwright (`sha256` email, 5 min TTL). Ignored when `VERCEL_ENV=production`. Never set on production.                                                                                                                                     |
| `E2E_ALLOW_DB_WRITES`   | Playwright last-resort verify | When `1`, e2e may flip `auth.user.email_verified` if no Redis URL is captured. Off by default.                                                                                                                                                                                                                                       |

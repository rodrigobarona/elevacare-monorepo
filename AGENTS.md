...
- Stripe webhook events follow a **two-file contract**: the handler (`apps/api/src/app/stripe/webhook/route.ts`) and the setup script (`infra/stripe/setup-webhooks.ts`). When adding or removing webhook events, update **both** files and re-run `pnpm stripe:setup:webhooks -- --url <ENDPOINT_URL> --apply`. See `.cursor/rules/stripe-webhooks.mdc` and `.cursor/skills/stripe-webhooks/SKILL.md`.
